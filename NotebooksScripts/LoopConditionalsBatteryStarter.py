...
soc_E = [0] * len(demand_P)  # <--- List to store state of charge, starting at 50 kWh
...
for i in range(len(demand_P)): # <--- Fill in the for loop to iterate over the range of demand_P length
    ...
    # filter for max battery power, cap it at max
    if bat_P[i] > max_power:
        bat_P[i] = max_power;
    ...
    # check if SoC is out of limits, and if so, max or min it out at limits
    if soc_E[i] > max_soc:
        soc_E[i] = max_soc
    elif soc_E[i] < min_soc:
        soc_E[i] = min_soc
...
print('the final state of charge is: ' + str(soc_E[-1])+' kWh') # <--- Add a print statement to display the final state of charge. 